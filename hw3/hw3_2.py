...
# initialize figure
fig, ax_stocks = plt.subplots(figsize=(10, 6))
ax_temp = ax_stocks.twinx()

# plot to axes
l1 = ax_stocks.plot(yahoo['Modified Julian Date'].values, yahoo['Stock Value'].values, c='purple', label='Yahoo! Stock Value')
l2 = ax_stocks.plot(goog['Modified Julian Date'].values, goog['Stock Value'].values, c='b', label='Google Stock Value')
l3 = ax_temp.plot(temps['Modified Julian Date'].values, temps['Max Temperature'].values, 'r--', label='NY Mon. High Temp')
...
ax_temp.legend(lines, labels, loc='center left', frameon=False, fontsize=10)
